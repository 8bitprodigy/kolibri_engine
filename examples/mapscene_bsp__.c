...
MakeWindingsForBrush(side_t *brush_sides)
{
    ...
    if (debug) {
        DBG_OUT("[BSP DEBUG] Building windings for brush with %d sides", side_count);
    }
    ...
    for (side_t *s = brush_sides; s; s = s->next) {
        ...
        if (debug) {
            DBG_OUT("[BSP DEBUG]   Side plane: normal=(%.2f,%.2f,%.2f) dist=%.2f, base winding has %d points",
        ...
        for (side_t *clip = brush_sides; clip && s->winding; clip = clip->next) {
            ...
            /* Clip to back side (inside the brush) */
            s->winding = ClipWindingToPlane(
                s->winding,
                g_plane_normals[clip->planenum],
            ...
            if (!s->winding && debug) {
                DBG_OUT("[BSP DEBUG]     Clipped away by plane %d: normal=(%.2f,%.2f,%.2f) dist=%.2f",
                        clip_count,
        ...
        if (debug && s->winding) {
            DBG_OUT("[BSP DEBUG]   Final winding has %d points", s->winding->numpoints);
        }
    }
...
BSP_Build(const MapData *map_data)
{
    ...
    DBG_OUT("[BSP] Building from %d brushes (QBSP algorithm)...", 
            map_data->world_brush_count);
    
    /* Convert all brushes to sides and build windings per brush */
    ...
    DBG_OUT("[BSP] Generated %d sides, %d planes, %d valid windings",
            map_data->world_brush_count * 6, g_num_planes, valid_count);
    
    /* Build the tree */
    DBG_OUT("[BSP] Building tree...");
    ...
    DBG_OUT("[BSP] Tree built: %d nodes, %d leaves", node_count, leaf_count);
    ...
    DBG_OUT("[BSP] Flattened: %d nodes, %d leaves, %d faces",
            tree->node_count, tree->leaf_count, tree->total_faces);
    
    /* Classify leaves */
    ClassifyLeaves(tree, map_data);
    
    int solid = 0, empty = 0;
    for (int i = 0; i < tree->leaf_count; i++) {
        if (tree->leaves[i].contents == CONTENTS_SOLID) solid++;
        else empty++;
    }
    
    DBG_OUT("[BSP] Classification: %d SOLID, %d EMPTY", solid, empty);
...
BSP_DebugDrawLeafBounds(const BSPTree *tree)
{
    ...
    for (int i = 0; i < tree->leaf_count; i++) {
        ...
        if (leaf->face_count == 0) continue;
        
        for (BSPFace *face = leaf->faces; face; face = face->next) {
            if (face->vertex_count < 3) continue;
            
            if (leaf->contents == CONTENTS_SOLID) {
                /* SOLID leaves: Draw filled orange polygons with alpha */
                Color color = (Color){255, 128, 0, 64};
                
                /* Draw as triangle fan */
                for (int v = 1; v < face->vertex_count - 1; v++) {
                    DrawTriangle3D(
                        face->vertices[0],
                        face->vertices[v],
                        face->vertices[v + 1],
                ...
                /* EMPTY leaves: Draw green wireframe, slightly inset */
                Color color = leaf->is_reachable ? 
                             (Color){0, 128, 255, 255} :  // Blue for reachable
                             (Color){0, 255, 0, 255};     // Green for empty
                
                /* Calculate face center */
                Vector3 face_center = {0, 0, 0};
                for (int v = 0; v < face->vertex_count; v++) {
                    face_center.x += face->vertices[v].x;
                    face_center.y += face->vertices[v].y;
                    face_center.z += face->vertices[v].z;
                }
                face_center.x /= face->vertex_count;
                face_center.y /= face->vertex_count;
                face_center.z /= face->vertex_count;
                
                /* Draw wireframe with slight inset */
                float inset = 0.01f;
                
                for (int v = 0; v < face->vertex_count; v++) {
                    int next = (v + 1) % face->vertex_count;
                    
                    Vector3 v1 = face->vertices[v];
                    Vector3 v2 = face->vertices[next];
                    
                    /* Move toward face center */
                    Vector3 to_center1 = Vector3Subtract(face_center, v1);
                    float len1 = Vector3Length(to_center1);
                    if (len1 > 0.001f) {
                        to_center1 = Vector3Scale(Vector3Normalize(to_center1), inset);
                        v1 = Vector3Add(v1, to_center1);
                    }
                    
                    Vector3 to_center2 = Vector3Subtract(face_center, v2);
                    float len2 = Vector3Length(to_center2);
                    if (len2 > 0.001f) {
                        to_center2 = Vector3Scale(Vector3Normalize(to_center2), inset);
                        v2 = Vector3Add(v2, to_center2);
                    }
